...
if (!$mysqli = getConectionDb()) {
    ...
    $existeSql = "select zonas from empresas where nombre='" . $json["nombre"] . "'";
    ...
    if ($result) {
        if ($result->num_rows > 0) {
            ...
            $insertSql = "insert into zonas (nombre,horario,color,descripcion,tiempo,tiempo_fraccion)"
                    . "values(?, ?, ?, ?, ?,?)";

            $stmt = $mysqli->prepare($insertSql);
            if ($stmt) {
                $stmt->bind_param("ssssss", utf8_decode($json["nombre"]), $json["horario"], $json["color"], utf8_decode($json["descripcion"]), $json["tiempo"], $json["tiempo_fraccion"]);
                $stmt->execute();

                if ($stmt->affected_rows > 0) {
                    echo "{success:true, message:'Zona Creada Correctamente.',state: true}";
                    $sql = "SELECT MAX(ID_ZONZ) AS M FROM ZONAS";
                    $result1 = coneccion()->query($sql);
                    $myrow = $result1->fetch_assoc();
                    $idGeo = $myrow["M"];
                    for ($i = 0; $i < count($coord); $i++) {
                        $xy = explode(",", $coord[$i]);
                        $consultaSql4 = "INSERT INTO zonas(id_zona,orden ,latitud, longitud)
